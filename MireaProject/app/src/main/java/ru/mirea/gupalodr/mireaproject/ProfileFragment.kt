...
class ProfileFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val sharedPref = requireContext().getSharedPreferences("profile", AppCompatActivity.MODE_PRIVATE)
        name.setText(sharedPref.getString("NAME", ""))
        age.setText(sharedPref.getInt("AGE", 0).toString())
        color.setText(sharedPref.getString("COLOR", ""))

        view.findViewById<Button>(R.id.button2).setOnClickListener {
            val editor = sharedPref.edit()
            editor.putString("NAME", name.text.toString());
            editor.putInt("AGE", age.text.toString().toInt());
            editor.putString("COLOR", color.text.toString());
            editor.apply();
        }
    }
